...
We formalize this as optimizing performance under a cost budget:
\begin{equation}
\max \; \mathbb{E}\left[S_{\text{final}}\right] \quad \text{s.t.} \quad \sum_{t=0}^{T-1} c_t \le B
\end{equation}
where $S_{\text{final}}$ is the terminal task score, $c_t$ is the per-step API cost, and $B$ is a per-episode cost budget.
This budgeted formulation reflects practical deployments where users must complete an episode within a fixed spend.
...
\item \textbf{Method}: We propose \method, which learns a dual-head value model (predicting score and cost separately) from offline trajectories and performs budget-aware multi-turn routing under a fixed per-episode cost budget (\S\ref{sec:learning}).
