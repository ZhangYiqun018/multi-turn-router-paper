...
Table~\ref{tab:main_results} reports in-distribution test results.
...
Table~\ref{tab:ood_results} reports OOD performance under semantic distribution shift (held-out ScienceWorld task types; held-out HLE subject categories).
...
\label{sec:analysis}
While the in-domain and out-of-domain results (Tables~\ref{tab:main_results}--\ref{tab:ood_results}) and ablations (Table~\ref{tab:ablation}) establish \method's effectiveness, we next ask a more diagnostic question: \emph{why} does it work?
...
Despite \method achieving better end performance (Tables~\ref{tab:main_results}--\ref{tab:ood_results}), its trajectories typically reach success with \emph{fewer} switches and \emph{lower} cumulative cost (e.g., on ScienceWorld: $\sim$5 switches for \method vs.\ $\sim$20 for Router-R1).
