...
\caption{Error-triggered switching and recovery. \textit{(Placeholder; we will finalize the description.)}}
\label{fig:error_switch_recovery}
\end{figure}

\paragraph{Error recovery patterns.}
\textit{Placeholder.}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/cost_switches.pdf}
\caption{Cost and switching behavior. \textit{(Placeholder; we will finalize the description.)}}
\label{fig:cost_switches}
\end{figure}

\paragraph{Cost-aware switching.}
\textit{Placeholder.}
...
Table~\ref{tab:ablation} summarizes ablations on ScienceWorld and HLE; remaining variants are pending final runs.
