...
\textbf{Single-Turn Routers (step-wise).} We adapt three single-turn routing methods---RouterDC \citep{chen2024routerdc}, EmbedLLM \citep{wang2024embedllm}, and Avengers \citep{lu2024avengers}---to the multi-turn setting by applying them independently at each step. Concretely, each router consumes the current history $h_t$ (treated as the ``query'') and selects a model $a_t$ from the pool based on its single-turn scoring rule.
