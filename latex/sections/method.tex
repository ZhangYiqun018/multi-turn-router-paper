...
We collect trajectories under a \textbf{random router policy} that uniformly samples from the model pool at each step: $\mu(a|h_t) = 1/|\mathcal{A}|$. This ensures coverage over the action space, which is critical for offline learning.

For each task variation, we run 5 random-router episodes and 3 baseline episodes (single-model runs). The combined dataset contains 137K training samples across both benchmarks.
...
\textbf{Random Router.} Uniformly samples from the model pool at each step: $\pi(a|h_t) = 1/|\mathcal{A}|$. This matches the random-router collection policy used to gather offline trajectories.

\textbf{Single-Turn Routers (episode-level).} We include three single-turn routing methods---RouterDC \citep{chen2024routerdc}, EmbedLLM \citep{wang2024embedllm}, and AvengersPro (our implementation of Avengers \citep{lu2024avengers})---as \emph{per-episode} routers: at the beginning of an episode, the router consumes the initial query context (history) and selects a model $a$ from the pool; the agent then uses the same model for all subsequent turns in the episode. This baseline is intentionally stronger than a fixed single-model choice (it can tailor the model to the query), but it cannot adapt to within-episode phase changes or error recovery, thereby highlighting the need for multi-turn routing.

\textbf{Router-R1.} A turn-level multi-turn router baseline. At each turn, a routing policy model selects which model to invoke based on the recent history (last 3 turns) and model descriptions. Following the Router-R1 method, we train a \texttt{Qwen2.5-7B-Instruct} policy model for routing. We include the policy model cost when reporting total cost.

\textbf{LLM Router.} A turn-level multi-turn router baseline that uses the same routing prompt/template as Router-R1, but directly uses \texttt{DeepSeek-V3.2} as the routing policy model (no training/fine-tuning).
