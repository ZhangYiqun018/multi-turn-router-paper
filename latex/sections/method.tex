...
\textbf{Random Router.} Uniformly samples from the model pool at each step: $\pi(a|h_t) = 1/|\mathcal{A}|$.

\textbf{Heuristic Router.} A rule-based policy: (1) use the strongest model (GPT-5) for the first step; (2) switch to the strongest model after detecting errors; (3) in late phase ($t \geq 0.67 \cdot T_{\max}$), use a value-oriented model; (4) otherwise, sample from cheap models.

\textbf{LLM Router.} Uses a policy LLM (DeepSeek-V3.2) to select models based on conversation context and model descriptions. The LLM is prompted with the last 3 turns and outputs a model selection with reasoning.

\textbf{Per-Model Ridge.} A simpler learned baseline: train independent Ridge regression models for each candidate, predicting score from history embeddings. At inference, select the model with highest predicted score minus $\lambda \cdot \text{predicted\_cost}$.
