...
\end{tikzpicture}
\caption{\textbf{Overview of \method.} \textbf{Left:} Offline trajectories are collected from a mixture of (i) a uniform (random) router that samples models at each turn and (ii) single-model runs on training tasks. \textbf{Center:} The history encoder $\phi$ encodes the dialogue context into $z_x$; the model encoder $\psi$ maps each candidate to $z_a$. An outcome estimator predicts the expected terminal score $\hat{s}$ for each history--model pair. \textbf{Right:} At inference, the router selects the model with the highest predicted outcome at each turn, while the episode is evaluated under the same per-episode budget and turn limit.}
